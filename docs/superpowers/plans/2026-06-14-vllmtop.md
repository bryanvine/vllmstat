...
**Goal:** Build `vllmtop`, an interactive, zero-infra terminal dashboard for a live vLLM server (concurrency, throughput, cache & KV-memory, latency percentiles, speculative-decode acceptance, per-GPU stats) — `nvtop` for vLLM.
...
**Tech Stack:** Python ≥3.10, Textual (TUI), httpx (async scrape), prometheus-client (text parser), nvidia-ml-py (GPU, import-guarded). Tooling: pytest + pytest-asyncio, ruff, pyright. Packaging: hatchling, `src/` layout, console entry `vllmtop`.

**Spec:** `docs/superpowers/specs/2026-06-14-vllmtop-design.md` (read it first).
...
- Create: `src/vllmtop/__init__.py`
- Create: `src/vllmtop/__main__.py`
...
[project.urls]
Homepage = "https://github.com/bryanvine/vllmtop"
Issues = "https://github.com/bryanvine/vllmtop/issues"

[tool.hatch.build.targets.wheel]
packages = ["src/vllmtop"]
...
git commit -m "chore: scaffold vllmtop package, packaging, and tooling"
```
...
**Files:**
- Create: `src/vllmtop/core/__init__.py` (empty)
- Create: `src/vllmtop/core/state.py`
...
`tests/test_state.py`:
```python
from vllmtop.core.state import Quantiles, GpuSample, GpuSnapshot, Snapshot
...
Expected: FAIL (`ModuleNotFoundError: vllmtop.core.state`).
...
```bash
git add src/vllmtop/core/__init__.py src/vllmtop/core/state.py tests/test_state.py
...
**Files:**
- Create: `src/vllmtop/core/parse.py`
...
```bash
git add src/vllmtop/core/parse.py tests/test_parse.py
...
**Files:**
- Create: `src/vllmtop/core/histogram.py`
...
```bash
git add src/vllmtop/core/histogram.py tests/test_histogram.py
...
**Files:**
- Create: `src/vllmtop/core/rates.py`
...
```bash
git add src/vllmtop/core/rates.py tests/test_rates.py
...
```bash
git add src/vllmtop/core/kv.py tests/test_kv.py
...
**Files:**
- Create: `src/vllmtop/core/history.py`
...
```bash
git add src/vllmtop/core/history.py tests/test_history.py
...
**Files:**
- Create: `src/vllmtop/core/metrics.py`
...
```bash
git add src/vllmtop/core/metrics.py tests/test_metrics.py
...
**Files:**
- Create: `src/vllmtop/providers/__init__.py` (empty)
- Create: `src/vllmtop/providers/vllm.py`
...
```bash
git add src/vllmtop/providers/__init__.py src/vllmtop/providers/vllm.py tests/test_provider_vllm.py
...
**Files:**
- Create: `src/vllmtop/providers/mock.py`
...
```bash
git add src/vllmtop/providers/mock.py tests/test_provider_mock.py
...
**Files:**
- Create: `src/vllmtop/providers/gpu.py`
...
```bash
git add src/vllmtop/providers/gpu.py tests/test_provider_gpu.py
...
```bash
git add src/vllmtop/config.py tests/test_config.py
...
**Files:**
- Create: `src/vllmtop/cli.py`
- Create: `src/vllmtop/snapshot_json.py`
...
```bash
git add src/vllmtop/cli.py src/vllmtop/snapshot_json.py tests/test_cli_once.py
...
```bash
git add src/vllmtop/format.py tests/test_format.py
...
Implement one function per panel returning a string (use `vllmtop.format` helpers). Each must read only `Snapshot`/`History` fields defined in Task 2/7. Required functions and the fields they surface (match the spec mockup §8.1):
...
```bash
git add src/vllmtop/render.py tests/test_render.py
...
**Files:**
- Create: `src/vllmtop/widgets.py`
- Create: `src/vllmtop/app.py`
...
```bash
git add src/vllmtop/widgets.py src/vllmtop/app.py tests/test_app.py
...
Run: `python -m vllmtop --once --json | python -m json.tool | head -40`
...
Run: `timeout 5 python -m vllmtop --url http://localhost:8000 --no-gpu` (Ctrl-C / timeout exits cleanly).
...
```bash
git add tests/test_resilience.py src/vllmtop/app.py
...
- [ ] **Step 2: Write `README.md`** — sections: what it is + screenshot (`docs/screenshot.svg`), why (gap vs Grafana/`vllm-top` printer), install (`pip install vllmtop`), usage (`vllmtop`, `--url`, `--mock`, `--once --json`, key bindings), what it shows (concurrency, throughput, cache & KV memory incl. compression, latency percentiles, spec-decode, GPU), requirements (Python ≥3.10; NVML optional), remote monitoring note (works without GPU access), license. No placeholders — write real prose.
...
```bash
cd /apps/vllm-tui
gh repo create bryanvine/vllmtop --public --source=. --remote=origin \
  --description "nvtop for vLLM — interactive terminal dashboard for vLLM serving performance" --push
```

- [ ] **Step 2: Verify**

Run: `gh repo view bryanvine/vllmtop --web` (or `gh repo view bryanvine/vllmtop`).
